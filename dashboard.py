...
<div class="content">
  ...
  <div class="panel-grid" id="panelGrid">
    ...
    <div class="section" data-panel="spread" draggable="true"><div class="section-header"><div class="sh-left"><h2>Spread efectivo</h2><p>Diferencia VWAP Compra &minus; Venta por profundidad (BOB).</p></div><div class="sh-controls"><button class="sh-ctrl-btn size-toggle" title="Alternar ancho">&#11036;</button><button class="sh-ctrl-btn drag-handle" title="Mover">&#10303;</button></div></div><div class="section-body"><div id="chartSpread"></div></div></div>
    ...
    <div class="section" data-panel="decile" draggable="true"><div class="section-header"><div class="sh-left"><h2>Curva de precio por decil</h2><p>VWAP acumulado 10%&ndash;100%. La &laquo;tijera&raquo; (precio Compra &gt; Venta a alta profundidad) suele revelar anuncios trampa.</p></div><div class="sh-controls"><button class="sh-ctrl-btn size-toggle" title="Alternar ancho">&#11036;</button><button class="sh-ctrl-btn drag-handle" title="Mover">&#10303;</button></div></div><div class="section-body"><div id="chartDecile"></div></div></div>
    <div class="section" data-panel="ratio" draggable="true"><div class="section-header"><div class="sh-left"><h2>Ratio Venta / Compra</h2><p>&gt;1 = m&aacute;s oferta que demanda.</p></div><div class="sh-controls"><button class="sh-ctrl-btn size-toggle" title="Alternar ancho">&#11036;</button><button class="sh-ctrl-btn drag-handle" title="Mover">&#10303;</button></div></div><div class="section-body"><div id="chartRatio"></div></div></div>
...
function rMerchants(){
  ...
  const renderSide=(label,arr)=>{
    ...
    return '<div class="merchants-side"><h3>'+label+'</h3><table class="merchants-table">'+
      '<thead><tr><th>#</th><th>Nick</th><th>Profundidad (USDT)</th><th>% del lado</th><th>VWAP (BOB)</th><th>Bancos</th><th>Trades/mes</th></tr></thead>'+
      '<tbody>'+rows+'</tbody></table></div>';
  };
  document.getElementById('topMerchants').innerHTML='<div class="merchants-grid">'+renderSide('Compra',data.BUY)+renderSide('Venta',data.SELL)+'</div>';
}
...
function rVolatility(c){
  ...
  Plotly.react('chartVolatility',[
    {x:v.map(d=>d.date),y:v.map(d=>d.buy_range),name:'Compra (rango)',type:'bar',marker:{color:c.green}},
    {x:v.map(d=>d.date),y:v.map(d=>d.sell_range),name:'Venta (rango)',type:'bar',marker:{color:c.orange}},
  ],{...L,barmode:'group',xaxis:{...L.xaxis,type:'category'},yaxis:{...L.yaxis,title:{text:'Rango VWAP 10% (BOB/USDT)',font:{size:10,color:c.textMuted}},tickformat:'.4f',rangemode:'tozero'},showlegend:true,height:chartHeight('chartVolatility')},PC);
}
...
function rFlow(c){
  ...
  Plotly.react('chartFlow',[
    {x:x,y:flow.map(f=>f.new_buy),name:'Nuevos Compra',type:'bar',marker:{color:hexToRgba(c.green,.5)}},
    {x:x,y:flow.map(f=>-f.gone_buy),name:'Desap. Compra',type:'bar',marker:{color:hexToRgba(c.green,.2)}},
    {x:x,y:flow.map(f=>f.new_sell),name:'Nuevos Venta',type:'bar',marker:{color:hexToRgba(c.orange,.5)}},
    {x:x,y:flow.map(f=>-f.gone_sell),name:'Desap. Venta',type:'bar',marker:{color:hexToRgba(c.orange,.2)}},
    {x:x,y:flow.map(f=>f.n_buy),name:'Total Compra',yaxis:'y2',line:{color:c.green,width:2},mode:'lines'},
    {x:x,y:flow.map(f=>f.n_sell),name:'Total Venta',yaxis:'y2',line:{color:c.orange,width:2},mode:'lines'},
...
document.getElementById('kpis').innerHTML='<div class="kpi"><div class="label">VWAP 10% Compra</div><div class="value buy">'+last.vb10.toFixed(4)+'</div><div class="sub">BOB por USDT</div></div><div class="kpi"><div class="label">VWAP 10% Venta</div><div class="value sell">'+last.vs10.toFixed(4)+'</div><div class="sub">BOB por USDT</div></div><div class="kpi"><div class="label">Spread efectivo</div><div class="value spread">'+last.sp10.toFixed(4)+'</div><div class="sub">'+(last.sp10/last.vb10*100).toFixed(2)+'% del precio</div></div><div class="kpi"><div class="label">Profundidad Compra</div><div class="value">'+(last.buy_depth/1e6).toFixed(2)+'M</div><div class="sub">'+last.buy_count+' anuncios &middot; USDT</div></div><div class="kpi"><div class="label">Profundidad Venta</div><div class="value">'+(last.sell_depth/1e6).toFixed(2)+'M</div><div class="sub">'+last.sell_count+' anuncios &middot; USDT</div></div><div class="kpi"><div class="label">Asimetr\u00eda</div><div class="value">'+last.depth_ratio.toFixed(2)+'\u00d7</div><div class="sub">Venta / Compra (profundidad)</div></div><div class="kpi"><div class="label">TC Oficial BCB</div><div class="value">'+meta.bcb_rate.toFixed(2)+'</div><div class="sub">Prima P2P vs oficial: +'+premium+'%</div></div>'+(meta.bcb_ref_venta!=null?'<div class="kpi"><div class="label">TC Referencial BCB (venta)</div><div class="value spread">'+meta.bcb_ref_venta.toFixed(2)+'</div><div class="sub">Compra: '+meta.bcb_ref_compra.toFixed(2)+' BOB/USDT &middot; Fuente: BCB</div></div>':'');
