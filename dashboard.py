...
DEFAULT_OUTPUT = Path("p2p_dashboard.html")
BCB_RATE = 6.96
...
<div class="footer"><strong>Binance P2P USDT/BOB</strong> &mdash; Investigaci&oacute;n Econ&oacute;mica &middot; <span id="footerText"></span></div>
...
function renderTimeSeries(v){currentView=v;const c=getC(),ts=datasets[v],x=makeDates(ts);rVwap(c,ts,x);rSpread(c,ts,x);rDepth(c,ts,x);rRatio(c,ts,x);rConc(c,ts,x)}
function replotAllCharts(){const c=getC(),ts=datasets[currentView],x=makeDates(ts);rVwap(c,ts,x);rSpread(c,ts,x);rDepth(c,ts,x);rRatio(c,ts,x);rConc(c,ts,x);rDecile(c);rBank(c)}
