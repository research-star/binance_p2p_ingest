...
## 2026-07-17 — Fase 2A: taxonomía V4 en dry-run, sin persistencia

**Perímetro.** Se implementó una capa V4 determinística sobre el texto y los
campos existentes. No se ejecutó `--reextraer`, no se modificaron los 74 JSON de
`static/`, no se reprocesaron PDFs y no hubo red/API/IA. `grupo` y `campos` V3
permanecen como compatibilidad hasta que Diego autorice la migración.

**Contrato.** 17 tipos visibles y 120 subtipos, clave estable
`type_id.subtype_id`, un evento dominante, secundarios buscables, tags, campos
estructurados y estados explícitos de tabla fuente. Financiamiento bancario y
corporativo se unifican visualmente en `financiamiento` sin mezclar bonos,
pagarés bursátiles, colocaciones ni pagos de valores.

**Conciliación.** Universo idéntico al auditado: 30.267 items, 74 meses,
2020-01-02..2026-07-15. Los totales V4 coinciden exactamente con V2 después de
sumar los dos tipos previos de financiamiento (1.851). En subtipos hay una
corrección conocida: cinco registros pasan de `uso_linea_credito` a
`contratacion_bancaria` porque V2 encontraba `uso` dentro de otras palabras
(por ejemplo `recursos`) sin exigir proximidad a “línea de crédito”. Auditorías: 509/509 del
CSV ChatGPT, 488/488 recuperaciones independientes y 2.972/2.972 recuperaciones
consolidadas. Los tres redireccionamientos quedan como dos pagarés privados y
un destino inicial de uso de fondos.

**Casos.** OLEUM es `capital_societario.aportes_capitalizacion`; la junta
extraordinaria queda secundaria y la ficha expone Bs 34.000.000 / 31-jul-2026.
Banco Solidario es `capital_societario.transferencia_accionaria`, pero la tabla
queda `detected_unparsed`, sin filas ni etiqueta verificada: el PDF exacto sigue
siendo gate de Fase 2B.

**Freno.** La implementación se detiene antes de persistir o publicar. El
procedimiento y las etiquetas de verificación viven en
`docs/asfi_taxonomy_v4.md`.

**Muestra estratificada.** El export cubre 119 estratos año×tipo (238 filas).
La revisión inicial dejó cuatro decisiones para Diego sin romper la
especificación aprobada: `asfi:2020-03-17:040` parece falso positivo de
autorización; `asfi:2020-04-28:013` parece rendimiento de fondo recuperable;
`asfi:2020-09-10:002` solo toma conocimiento de informes en asamblea; y
`asfi:2020-08-31:001` podría afinarse a pago realizado. Se conservan como V2
hasta decisión de producto, salvo la corrección mecánica de cinco préstamos.
